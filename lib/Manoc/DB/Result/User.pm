...
__PACKAGE__->has_many( map_user_group => 'Manoc::DB::Result::UserGroupMap', 'user_id' );
__PACKAGE__->many_to_many( groups => 'map_user_group', 'group' );

sub roles {
  my $self = shift;
  return $self->search_related('groups')
    ->search_related('group')
    ->search_related('group_roles')
    ->search_related('role', {}, {distinct => 1});
}
